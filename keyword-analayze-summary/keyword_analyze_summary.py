...
COL_CONTENT_TAGGED_SENT = "diary_sentences"
COL_CONTENT_RAW_SENT = "diary_raw_sentences"
...
def content_word_tokenize(text):
    ...
    return tagged_words,tagged_sentences,sentences
...
def plot_bar(tf_idf_data,title,save_name):
    ...
    plt.tight_layout()
    plt.savefig(f'./keyword-analayze-summary/images/{save_name}')
...
#data-processing
df[COL_CONTENT_TAGGED_SENT] = ""
df[COL_CONTENT_RAW_SENT] = ""
for index, value in enumerate(df[COL_CONTENT]):
    temp = content_word_tokenize(value)
    df[COL_CONTENT][index] = temp[0]
    df[COL_CONTENT_TAGGED_SENT][index] = temp[1]
    df[COL_CONTENT_RAW_SENT][index]=temp[2]
...
for index, value in enumerate(df[COL_CONTENT]):
    ...
    for key in tf_data.keys():
        ...
        tt.append([key,tf_data[key],idf_data[key],tf_idf_data[key]])

    tt= sorted(tt, key=lambda x: x[3], reverse=True)
    new_df = pd.DataFrame(tt, columns=["token", "tf score","idf score","tf-idf score"])
    new_df.to_csv(f"keyword-analayze-summary/keyword/{str(df[COL_DATE][index])}.csv",index=False)

    yy=[]
    for j,sentence in enumerate(df[COL_CONTENT_TAGGED_SENT][index]):
        sum=0
        for word in sentence:
            if word in tf_idf_data.keys():
                sum = sum + tf_idf_data[word]
        yy.append([df[COL_CONTENT_RAW_SENT][index][j],sum])
        #print(sentence,sum)

    yy = sorted(yy, key=lambda x: x[1], reverse=True)
    new_df = pd.DataFrame(yy, columns=["sentence", "tf-idf-sum"])
    new_df.to_csv(f"keyword-analayze-summary/summary/{str(df[COL_DATE][index])}.csv",index=False)
